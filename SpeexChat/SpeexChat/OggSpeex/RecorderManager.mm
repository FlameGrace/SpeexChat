...
AudioQueueLevelMeterState *leveeterStates;
...
- (void)startRecordingInFilePath:(NSString *)filePath
{
    ...
    if (!leveeterStates)
    {
        leveeterStates = (AudioQueueLevelMeterState *)malloc(sizeof(AudioQueueLevelMeterState) * 1);
    }
    [self endRecorderTimer];
    [self endLeveeterTimer];
    [self timerTimeout];
    [self timerLeveeter];
}
...
- (void)updateLeveeter:(id)sender {
    if (self.delegate) {
        ...
        AudioQueueGetProperty(mAQRecorder->Queue(), kAudioQueueProperty_CurrentLevelMeter, leveeterStates, &dataSize);
        if ([self.delegate respondsToSelector:@selector(leveeterChanged:)]) {
            [self.delegate leveeterChanged:leveeterStates[0].mPeakPower];
        }
...
- (void)endLeveeterTimer
{
    if(self.timerLeveeter)
    {
        [self.timerLeveeter invalidate];
        self.timerLeveeter = nil;
    }
    return;
}
-  (NSTimer *)timerLeveeter
{
    if(!_timerLeveeter)
    {
        _timerLeveeter = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(updateLeveeter:) userInfo:nil repeats:YES];
        [[NSRunLoop currentRunLoop]addTimer:_timerLeveeter forMode:NSDefaultRunLoopMode];
        [_timerLeveeter fire];
    }
    return _timerLeveeter;
}
